...
nb_note = 20
nb_none = int((1 - nb_note/nb_plats)*22)
notes = [None for i in range(nb_none)] + [i for i in range(11)]
...
faible implique une similarité élevée. On prend, ainsi, le complément à 1 de l'écart absolu moyen divisé par 10 (les notes de l'application
étant sur 10, l'écart absolu moyen est compris entre 0 et 10).
"""

for i in range(len(liste_df_users)):
    score_similarite.append((10 - np.mean(np.absolute(df_user_ref[df_user_ref['plat'].isin(liste_df_users[i]['plat'])]['note'].to_numpy()-liste_df_users[i]['note'].to_numpy())))/10)
